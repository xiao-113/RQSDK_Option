...
- **虚值额**：`4.0 - 3.866 = 0.134`
  ...
  - 实际需求保证金：`1,059,450元`（300手）→ 每手约 `3,531.5元`。
  - 若忽略最低保证金，解算比例：3,531.5≈170+(3.866×10,000×*X*−0.134×10,000)**解得保证金比例 `X ≈ 10%`**（

2. 认沽期权（行权价3.8）

- **虚值额**：`3.866 - 3.8 = 0.066`
- 实际需求保证金：`1,185,750元`（300手）→ 每手约 `3,952.5元`。
- 反向推导：3,952.5≈248+(3.8×10,000×*X*−0.066×10,000)**解得保证金比例 `X ≈ 10%`**。
