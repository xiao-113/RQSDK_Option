...
def before_trading(context):
    ...
    #获取目标行权价的看涨和看跌期权合约
    context.s1 =  options.get_contracts(underlying='000300.XSHG', maturity=context.order_current_month, strike=strike)[0]
    context.s2 = options.get_contracts(underlying='000300.XSHG', maturity=context.order_current_month, strike=strike)[1]
    print(f'今日目标认购/沽平值合约 {context.s1} /{context.s2} @行权价{strike}')
