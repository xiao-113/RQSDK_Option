...
def init(context):
    ...
    print(f"前一个交易日 {latest_trading_date}")
    ...
    print(f"行权价 {strike}")
    #获取当月目标行权价的合约list
    option_list = options.get_contracts(underlying='510300.XSHG', maturity=context.current_month, strike=strike)
    print(f'将订阅合约： {option_list}')
    #获取目标行权价的看涨和看跌期权合约
    ...
    if not context.initialized:
        ...
        if normalized_time == time(9, 45):
            ...
            if not all(rqdatac.instruments(c).days_to_expire() >= 0 for c in [context.s1, context.s2]):
                ...
                new_option_list = options.get_contracts(underlying='510300.XSHG', maturity=next_month, strike=new_strike)
                # 开新仓
                context.s1 = new_option_list[0] 
                #context.s1 = rqdatac.instruments(new_option_list[0], market='cn').product_name
                context.s2 = new_option_list[1]
                #context.s2 = rqdatac.instruments(new_option_list[1], market='cn').product_name
