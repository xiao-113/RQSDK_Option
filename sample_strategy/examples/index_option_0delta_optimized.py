...
def init(context):
    ...
    #当月or次月合约认购侧
    C_option_list = [] #初始化
    for C_strike in call_strike_list:
        temp_list = options.get_contracts(underlying='000300.XSHG', option_type = 'C', 
                                            maturity=context.current_month, strike = C_strike,
                                            trading_date = context.now.date())
        C_option_list = C_option_list + temp_list    
    #当月or次月合约认沽侧
    P_option_list = [] #初始化
    for P_strike in put_strike_list:
        temp_list = options.get_contracts(underlying='000300.XSHG', option_type = 'P', 
                                            maturity=context.current_month, strike = P_strike,
                                            trading_date = context.now.date())
        P_option_list = P_option_list + temp_list
    ...
    if not C_option_list or not P_option_list:
        ...
        for C_option in C_option_list:
            ...
            for P_option in P_option_list:
                ...
                if delta_diff < min_delta_diff:
                    ...
                    best_C_option = C_option
                    best_P_option = P_option
                    best_C_delta = C_delta
                    best_P_delta = P_delta
        # 输出结果
        print(f"最接近delta中性的组合:认购期权: {best_C_option}, Delta: {best_C_delta:.4f} \
            / 认沽期权: {best_P_option}, Delta: {best_P_delta:.4f}")
        ...
        subscribe([context.s1, context.s2]) #初始化订阅合约池
...
def before_trading(context):
    ...
    for C_strike in call_strike_list:
        temp_list = options.get_contracts(underlying='000300.XSHG', option_type = 'C', 
                                            maturity=context.order_current_month, strike = C_strike,
                                            trading_date = context.now.date())
        C_option_list = C_option_list + temp_list    
    #当月or次月合约认沽侧
    P_option_list = [] #初始化
    for P_strike in put_strike_list:
        temp_list = options.get_contracts(underlying='000300.XSHG', option_type = 'P', 
                                            maturity=context.order_current_month, strike = P_strike,
                                            trading_date = context.now.date())
        P_option_list = P_option_list + temp_list
